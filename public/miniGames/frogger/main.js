...
function salirDelJuego() {
    if (window.parent && window.parent.cerrarMinijuego) {
        ...
        alert("No se puede cerrar: No est├ís dentro del juego principal.");
    }
}
    ...
    lanes.forEach(lane => {
        if (lane.type === 'road') {
            for (let i = 0; i < lane.count; i++) {
                let startX;
                // usar semilla del servidor si existe, sino local
                if (state && state.seeds && state.seeds.length > seedIndex) {
                    startX = state.seeds[seedIndex] * canvas.width;
                    seedIndex++;
                } else {
                ...
                // calcular posicion actual basada en tiempo transcurrido
                if (state && state.startTime) {
                    // estimar frames transcurridos (60fps = ~16.66ms per frame)
                    const msElapsed = currentTime - state.startTime;
                    const framesElapsed = msElapsed / 16.666;
                    const totalMove = lane.speed * framesElapsed;

                    // ajustar posicion con wrapping simple
                    currentX = (startX + totalMove) % canvas.width;
